...
SCALE = 225  # Вводим 225, чтобы увидеть радиопузырь Земли
...
def scale_galaxy():
    """Шкалирование размеров галактики на основеразмера радиопузыря (scale)."""
...
def detect_prob(disc_vol_scaled):
    ...
    # Отношение цивилизаций к шкалированному объему галакт
    ratio = NUM_CIVS / disc_vol_scaled
    if ratio < 0.002:  # установить очень низкие соотношения равными нулевой вероятности
        detection_prob = 0
    elif ratio >= 5:  # установить верхние соотношения  равными единичной вероятности
        detection_prob = 1
    else:
...
def spirals(b, r, rot_fac, fuz_fac, arm):
    """Построить спиральные рукава для изображения в tkinter, используя формулу логарифмической спирали.
    ...
    fuz_fac - случайный сдвиг в позиции звезды в рукаве, применительно к переменной 'fuzz'
...
def star_haze(disc_radius_scaled, density):
    ...
    disc_radius_scaled = радиус галактического диска, прошкалированный в плотность радиопузыря
...
def main():
    """Рассчитать вероятность обнаружения и изобразить галактику и статистику."""
    ...
    c.create_text(-455, -330, fill='white', anchor='w',
                  text='Диаметр радиопузыря = {} св. лет'.format(SCALE))
